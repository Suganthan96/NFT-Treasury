...
// Email configuration will be created per request for better reliability
...
app.post('/api/bitbadges-webhook', express.json(), (req, res) => {
  ...
  try {
    ...
    const email = metadata?.email || req.body.email || 'your-email@example.com';
    const discord = metadata?.discord || req.body.discord || null;
